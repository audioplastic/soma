classdef gtfProc < soma.omeProc
    ...
    properties(SetAccess = 'public')
        ...
        bmm_filterMethod = 'Slaney'; %Brown or Slaney filtering
    end
    ...
    methods
        ...
        function [value, obj] = get.bmm(obj)
            ...
            if strcmpi(obj.bmm_filterMethod,'Brown') %modified Brown mex file
%                 tic
                value = soma.TURBOgammatone(obj.ome, 4, obj.cf, b, T); %gt filter (4 = 4th order)
%                 toc
...
%                 toc
        ...
        function obj = set.bmm_filterMethod(obj,value)
            assert (strcmpi(value,'Brown') || strcmpi(value,'Slaney'),...
                'Filtering method must be Slaney (native) or Brown (mex)')
            obj.bmm_filterMethod = value;
        end
    end
end
