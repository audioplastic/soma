...
%                 'Color', [0 0 0])                        
            ...
            %*** 2D subplot ***
            axes('Position',[ml mb mw mh])
%             set(get(gca,'Children'), 'Color', [1 1 1])
            pcolor(obj.tAxis*1e3, 1:obj.bmm_nChans, obj.ihc');
            shading interp; colormap bone
            ylabel('chan #');
            title(['Spiking prob: Compression = ',obj.ihc_cmpType])
            set(gca,'XTick', [], 'XTickLabel', [])
            
            %*** RMSc ***
            axes('Position',[ml+mw+hspc mb 1-mw-hspc-1.5*ml mh])
            barh(1:obj.bmm_nChans, obj.ihc_RMSc, 'BarWidth',1)
            ...
            %*** RMSt ***
            axes('Position',[ml ml mw 1-mh-2.3*ml])            
            plot(obj.tAxis*1e3, obj.ihc_RMSt, 'k')
            xlabel('time [ms]');
            ylabel('RMS');
            grid on
